...
import { calcRICE, riceLabel, EFFORT_OPTIONS } from "@/lib/utils"
...
export default function RiceForm() {
  const [form, setForm] = useState({
    ...
    reach: 0, impact: 40, confidence: 80, effort: 1, evidence: "",
  })
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label className="text-xs text-zinc-500 block mb-1">Impact % — share of reach meaningfully affected</label>
          <div className="flex items-center gap-2">
            <input className={field} type="number" min={1} max={100} value={form.impact} onChange={e => setForm(f => ({ ...f, impact: Number(e.target.value) }))} />
            <span className="text-sm text-gray-500 shrink-0">%</span>
          </div>
        </div>
        <div>
          <label className="text-xs text-zinc-500 block mb-1">Confidence % — evidence quality</label>
          <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <select className={field} value={form.effort} onChange={e => setForm(f => ({ ...f, effort: Number(e.target.value) }))}>
            {EFFORT_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label} (÷{o.value})</option>)}
          </select>
        </div>
        <div className="md:col-span-2">
      ...
      <div className="flex items-center justify-between pt-2">
        <div className="flex items-center gap-3">
          <span className="text-zinc-500 text-xs font-mono">({form.reach} × {form.impact}% × {form.confidence}%) ÷ {form.effort} =</span>
